...
impl Searcher<'_> {
    ...
    pub fn singular_search(
        ...
        if entry.kind == NodeKind::UpperBound
            || depth <= 6
            || entry.depth < depth - 1
        ...
        let singular_window = Window::null(entry.eval - 25 * depth);
        let v = self.null_search(position, singular_window, depth / 2 - 1, Some(entry.mv))?;
