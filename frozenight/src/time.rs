...
impl TimeManager {
    pub fn new(board: &Board, time: TimeConstraint) -> Self {
        ...
        let initial_soft_deadline = (clock.saturating_sub(time.increment) / (2 * mtg))
    ...
    pub fn update(&mut self, info: &SearchInfo) -> ControlFlow<()> {
        match self.soft_deadline {
            ...
            Some(ref mut deadline) => {
                ...
                // extend time if eval drops significantly (20 cp) and best move is not capture
                if info.eval < self.prev_best_eval - 100
                    && self.board.color_on(info.best_move.to) != Some(!self.board.side_to_move())
                {
                    *deadline += self.soft_deadline_increment;
                }
                // extend time if best move changed
                if self.prev_best_move != INVALID_MOVE && self.prev_best_move != info.best_move {
                    *deadline += self.soft_deadline_increment;
                }
